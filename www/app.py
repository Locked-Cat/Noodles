...
async def response_factory(app, handler):
    async def response(request):
        ...
        if isinstance(r, web.StreamResponse):
            ...
            c, m = r
            if isinstance(c, int) and c >= 100 and c < 600:
                return web.Response(c, str(m).encode('utf-8'))
...
async def init(loop):
    await orm.create_connection_pool(loop, host='127.0.0.1', port=3306, user='root', password='root', db='noodles')
    app = web.Application(loop=loop, middlewares=[response_factory])
    init_jinja2(app, filters=dict(datetime=datetime_filter))

    noodleFramework.add_routes(app, 'handlers')
    noodleFramework.add_static(app)
